...
@implementation PlusHController

- (void)windowDidLoad {
    [super windowDidLoad];
    
    // Implement this method to handle any initialization after your window controller's window has been loaded from its nib file.
    self.window.title = @"预览图2208x1242";
...
-(void)savePic{
    ...
    [panel setNameFieldStringValue:@"2208x1242"];
    ...
    [panel beginSheetModalForWindow:self.window completionHandler:^(NSInteger result){
        if (result == NSFileHandlingPanelOKButton) {//NSFileHandlingPanelOKButton
            ...
            NSData * data = [ZLImage compressedImageDataWithImg:[ZLImage resizeImage2:image forSize:CGSizeMake(2208,1242)] rate:0.4];
